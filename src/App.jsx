...
function App() {
  ...
  const projects = [
    { tag: 'E-commerce', title: 'Capshop', desc: 'Boutique en ligne de mode spécialisée dans la vente de casquettes.', tech: ['JavaScript', 'PHP', 'Symfony'], img: '/images/capshop.webp', url: 'https://capshop.benjaminpelissier.com/' },
  ...
  return (
    <div className="app-root" style={{ fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", background: c.bg, color: c.text, minHeight: '100vh', transition: 'all 0.3s ease' }}>

      {/* ── NAV ── */}
      <nav className="top-nav" style={{ position: 'sticky', top: 0, zIndex: 100, background: dark ? 'rgba(26,26,46,0.95)' : 'rgba(248,249,250,0.95)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', borderBottom: `1px solid ${c.border}`, padding: '0 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', minHeight: 64 }}>
        ...
        <button type="button" className="mobile-menu-toggle" aria-label={menuOpen ? 'Fermer le menu' : 'Ouvrir le menu'}
        ...
        <div className={`nav-actions ${menuOpen ? 'open' : ''}`} style={{
          ...
          gap: 4,
          alignItems: 'center',
          background: menuOpen ? (dark ? 'rgba(10,10,10,0.98)' : 'rgba(255,255,255,0.98)') : 'transparent',
          borderRadius: menuOpen ? 18 : 0,
          padding: menuOpen ? '12px 16px' : 0,
        }}>
          {navItems.map(n => (
          ...
          {[
            { href: 'https://github.com/BenjaminP17', icon: <GitHubIcon /> },
            { href: 'https://www.linkedin.com/in/benjamin-pelissier/', icon: <LinkedInIcon /> },
            { href: 'https://calendly.com/benjpls/30min', icon: <CalendlyIcon /> },
          ].map(({ href, icon }) => (
      ...
      {/* ── HERO ── */}
      <section id="hero" className="hero-section" style={{ minHeight: '92vh', position: 'relative', display: 'flex', alignItems: 'center', overflow: 'hidden' }}>
        ...
        <div className="hero-inner" style={{ position: 'relative', zIndex: 1, maxWidth: 1200, margin: '0 auto', padding: '60px 48px', width: '100%' }}>
          <div className="hero-card" style={{ background: 'rgba(255,255,255,0.12)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.25)', borderRadius: 28, padding: '56px 60px', boxShadow: '0 8px 32px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.35)', display: 'grid', gridTemplateColumns: '1fr 380px', gap: 48, alignItems: 'center' }}>
            ...
            <div>
              ...
              <h1 className="hero-title" style={{ fontWeight: 700, lineHeight: 1.1, margin: '0 0 20px', letterSpacing: '-2px', color: '#ffffff' }}>
      ...
      {/* ── SERVICES ── */}
      <section id="services" className="section services-section" style={{ background: dark ? 'rgba(255,255,255,0.02)' : '#f1f3f4', padding: '96px 64px' }}>
      ...
      <section id="projects" className="section projects-section" style={{ padding: '96px 64px', background: c.bg }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ marginBottom: 64, display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
            ...
            <a href="https://lab.benjaminpelissier.com" style={{ fontSize: 14, fontWeight: 600, color: c.accent, textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 6, padding: '8px 18px', borderRadius: 20, border: `1.5px solid ${c.border}`, background: dark ? 'rgba(138,180,248,0.06)' : 'rgba(26,115,232,0.04)', transition: 'all 0.2s', whiteSpace: 'nowrap' }}
      ...
      {/* ── CONTACT ── */}
      <section id="contact" className="section contact-section" style={{ position: 'relative', padding: '96px 64px', overflow: 'hidden' }}>
        ...
        <div className="contact-inner" style={{ position: 'relative', zIndex: 1, maxWidth: 680, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 40 }}>
            ...
            <h2 className="section-title" style={{ fontWeight: 700, letterSpacing: '-1.5px', margin: '0 0 16px', color: '#fff' }}>Parlons de votre projet</h2>
          ...
          <div style={{ textAlign: 'center', marginBottom: 32 }}>
            <a href="https://calendly.com/benjpls/30min" target="_blank" rel="noopener noreferrer"
              style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: '#fff', color: '#1a6cf5', padding: '14px 28px', borderRadius: 24, fontSize: 15, fontWeight: 700, textDecoration: 'none', boxShadow: '0 4px 20px rgba(0,0,0,0.2)', transition: 'all 0.2s' }}
              onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 8px 28px rgba(0,0,0,0.25)'; }}
              onMouseLeave={e => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = '0 4px 20px rgba(0,0,0,0.2)'; }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>
            ...
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', marginTop: 10 }}>Gratuit · Sans engagement</div>
          </div>

          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 32 }}>
            <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.2)' }}></div>
            <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', fontWeight: 500 }}>ou</span>
            <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.2)' }}></div>
          </div>
          {formSent ? (
            ...
            <div className="contact-form-card" style={{ background: 'rgba(255,255,255,0.12)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', borderRadius: 24, border: '1px solid rgba(255,255,255,0.25)', boxShadow: '0 8px 32px rgba(0,0,0,0.2), inset 0 1px 0 rgba(255,255,255,0.3)' }}>
              {formError && (
      ...
      {legalOpen && (
        <div onClick={() => setLegalOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <div onClick={e => e.stopPropagation()} className="modal-content" style={{ background: c.bgCard, borderRadius: 20, maxWidth: 620, width: '100%', border: `1px solid ${c.border}`, boxShadow: '0 24px 64px rgba(0,0,0,0.25)', position: 'relative' }}>
            <button onClick={() => setLegalOpen(false)} style={{ position: 'absolute', top: 20, right: 20, background: dark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)', border: 'none', color: c.textMuted, width: 36, height: 36, borderRadius: '50%', cursor: 'pointer', fontSize: 18, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
            <h2 style={{ fontSize: 28, fontWeight: 700, margin: '0 0 32px', color: c.text, letterSpacing: '-0.5px' }}>Mentions légales</h2>
            ...
            {[
              {
                ...
                lines: [
                  ...
                  'Métropole Lilloise, France',
                  'Email : benjpls@gmail.com',
                ]
              },
              {
                title: 'Hébergeur',
              ...
              },
              {
                ...
                lines: [
                  "L'ensemble du contenu de ce site (textes, images, design) est la propriété exclusive de Benjamin Pelissier. Toute reproduction, même partielle, est interdite sans autorisation préalable.",
                ]
              },
              {
                title: 'Données personnelles',
                lines: [
                  "Les informations collectées via le formulaire de contact (nom, email, message) sont utilisées uniquement pour répondre à vos demandes. Elles ne sont ni cédées ni revendues à des tiers. Conformément au RGPD, vous disposez d'un droit d'accès, de rectification et de suppression en contactant benjpls@gmail.com.",
                ]
              },
            ].map(({ title, lines }) => (
              <div key={title} style={{ marginBottom: 28 }}>
                <h3 style={{ fontSize: 14, fontWeight: 600, color: c.accent, textTransform: 'uppercase', letterSpacing: 1, margin: '0 0 10px' }}>{title}</h3>
                {lines.map((line, i) => (
                  <p key={i} style={{ fontSize: 14, color: c.textMuted, lineHeight: 1.7, margin: '0 0 2px' }}>{line}</p>
                ))}
              </div>
            ))}
          </div>
      ...
      {/* ── FOOTER ── */}
      <footer className="site-footer" style={{ padding: '32px 64px', borderTop: `1px solid ${c.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: c.bg }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
          <div style={{ fontSize: 14, color: c.textMuted }}>© 2026 Benjamin Pelissier</div>
          <button onClick={() => setLegalOpen(true)} style={{ background: 'none', border: 'none', fontSize: 13, color: c.textMuted, cursor: 'pointer', fontFamily: 'inherit', padding: 0, textDecoration: 'underline', textUnderlineOffset: 3, transition: 'color 0.2s' }}
        ...
        <div style={{ display: 'flex', gap: 24 }}>
          {[
            { href: 'https://github.com/BenjaminP17', label: 'GitHub', icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0 1 12 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z"/></svg> },
            { href: 'https://www.linkedin.com/in/benjamin-pelissier/', label: 'LinkedIn', icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg> },
            { href: 'https://calendly.com/benjpls/30min', label: 'Calendly', icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg> },
          ].map(({ href, label, icon }) => (
